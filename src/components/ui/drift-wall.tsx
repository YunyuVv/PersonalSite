...
export interface DriftWallItem {
  ...
  title?: string;
  href?: string;
  /** 副标题（如「12:30 · 86万播放」），默认隐藏，悬停/激活时显现 */
  subtitle?: string;
  /** 左上角平台角标文案 */
  badge?: string;
  /** 角标背景色 */
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className={innerClass}>
        ...
        {item.badge && (
          <span
            className="pointer-events-none absolute left-2 top-2 z-10 rounded-md px-2 py-0.5 text-[11px] font-medium text-white shadow-sm"
            style={{ backgroundColor: item.badgeColor ?? "rgba(0,0,0,0.55)" }}
          >
            {item.badge}
          </span>
        )}
        ...
        {(item.title || item.subtitle) && (
          <span className="pointer-events-none absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-black/85 via-black/30 to-transparent p-2.5">
            {item.title && (
              <span className="block text-xs font-semibold leading-snug text-white line-clamp-2">
                {item.title}
              </span>
            )}
            {item.subtitle && (
              <span className="mt-0.5 block text-[10px] text-white/75 opacity-0 transition-opacity duration-300 group-[.is-active]/tile:opacity-100">
                {item.subtitle}
              </span>
            )}
          </span>
